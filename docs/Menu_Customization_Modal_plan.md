...
Summary:
- Wired the Menu Customization Modal trigger from the sidebar footer and mounted the modal at the layout root for global availability.
...
Testing Notes:
- Verified the button is discoverable via [aria-label="Customize menu"].
- Confirmed modal opens/closes, Save/Cancel/Reset flows work with persisted state.
- No visual regressions; styles preserved.
